...
//        public string PutArtWork(clsAllWork prNewWork, clsAllWork prExistingWork)
        {
            try
            {
                int lcRecCount = clsDbConnection.Execute(
                    "UPDATE Work SET WorkType = @WorkType, Date = @Date, Value = @Value, Width = @Width, Height = @Height, Type = @Type, Weight = @Weight, Material = @Material WHERE Name = @Name AND ArtistName = @ArtistName",
                    prepareWorkParameters(prWork));
                if (lcRecCount == 1)
        ...
        public string DeleteArtWork(clsAllWork prWork)
        {
            try
            {
                int lcRecCount = clsDbConnection.Execute(
        ...
        public string DeleteArtist(object prKey)
        {
            try
            {
                Dictionary<string, object> par = new Dictionary<string, object>() { { "Name" , prKey} };
                int lcRecCount = clsDbConnection.Execute(
